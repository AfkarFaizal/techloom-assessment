/**
 * Concurrency test for Task 01:
 * Verifies that concurrent checkout requests on a limited stock item
 * never result in overselling (Pessimistic MongoDB transaction safety).
 */
...
async function runConcurrencyTest() {
  console.log('--- Starting Concurrency Test for Task 01 ---');
  try {
    // 1. Create a product with strictly 2 items in stock
    const createRes = await request('POST', '/products', {
      name: 'Flash Sale Test Item',
      price: 25.0,
      stock: 2,
    });
    ...
    // 2. Launch 10 simultaneous checkout attempts for 1 item each
    ...
    if (successes.length === 2 && conflicts.length === 8 && prodRes.body.available === 0) {
      console.log('TEST PASSED: No overselling occurred! Concurrency lock is 100% verified.');
      process.exit(0);
    } else {
      console.error('TEST FAILED: Unexpected distribution of successes/failures.');
      process.exit(1);
    }
  } catch (err) {
